...
## UX — Menu de navegação lateral ✅ (código pronto, checklist manual pendente)

**Objetivo**: dar às páginas autenticadas (`/conteudo`, `/aprovacao`, `/admin/*`) uma navegação lateral compartilhada, já que cada uma tinha seu próprio cabeçalho solto e não havia como transitar entre `/conteudo` e `/aprovacao` sem editar a URL manualmente. Melhoria de UX transversal, fora da numeração de milestones do MVP — spec e plano em [docs/superpowers/specs/2026-07-04-sidebar-navegacao-design.md](docs/superpowers/specs/2026-07-04-sidebar-navegacao-design.md) e [docs/plans/2026-07-04-sidebar-navegacao.md](docs/plans/2026-07-04-sidebar-navegacao.md).
...
- [x] Itens de menu agrupados em "Operação" (fila de posts/aprovação, todos os papéis com acesso) e "Administração" (admin/artistas/contas/usuários, só admin) — espelha exatamente `roleAllowsRoute()` de `lib/supabase/proxy.ts` (`components/dashboard/nav-items.ts`).
- [x] `AppSidebar` com estado ativo destacado, colapso em ícone-only, drawer off-canvas no mobile, e rodapé com nome/papel do usuário + logout — substituindo os botões "Sair" e badges de papel duplicados que existiam em `/conteudo` e `/aprovacao`.
...
**Concluído quando**: um usuário de cada papel consegue navegar entre as páginas às quais tem acesso pela sidebar, sem digitar URLs. *(Código commitado, revisado e mergeado na `main`. `npm run build`, `npx tsc --noEmit` e `npm run lint` rodam limpos (só os mesmos warnings pré-existentes de `<img>`/`alt` do M5). Falta rodar o checklist manual contra um projeto Supabase linkado, testando os 3 papéis de verdade no navegador: itens de menu corretos por papel, colapso/persistência da sidebar, drawer mobile, e logout — mesma limitação de "checklist manual pendente" de todos os milestones anteriores.)*
