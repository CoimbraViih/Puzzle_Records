...
- **Zernio validado com publicação real.** O adapter (`lib/publishing/zernio.ts`) foi escrito inicialmente sem documentação real da API (stub best-effort do M7), reescrito contra a doc pública (`docs.zernio.com`) e depois **testado com 2 publicações reais bem-sucedidas no Instagram** (conta de teste `@althorya.ai`, 09/07/2026) — ver detalhe técnico acima. 3 divergências entre a doc pública e o comportamento real foram corrigidas nesse processo (campo `filename` obrigatório no presign, resposta assíncrona de `POST /posts`, e um bug de código nosso com `??` vs `||` numa env var vazia). Ainda falta: testar vídeo, conectar a conta oficial `@puzzlerecordss`, e fechar o débito técnico de idempotência descrito acima antes do go-live real. A `PublishingProvider` isolada garante que esses ajustes, ou uma troca de fornecedor, sejam incrementais, não um redesenho.
