...
- **API real do Zernio** (auditada em 09/07/2026 contra `docs.zernio.com`, ver `lib/publishing/zernio.ts`): base `https://zernio.com/api/v1`, auth `Bearer sk_...`. Fluxo de mídia é upload próprio deles (`POST /media/presign` → `PUT` na URL retornada → `publicUrl` referenciado em `mediaItems`), não aceita URL externa direta. `POST /posts` retorna um `id` interno (guardado em `posts.zernio_post_id`, migration `0011`) — é esse `id`, não o link público (`post_url`), que a API de analytics (`GET /analytics?postId=`) exige depois. TikTok resolve o link publicado de forma assíncrona (webhook `post.tiktok.url_resolved`, ainda não implementado) — hoje isso vira falha explícita em vez de gravar link vazio.
...
- **Zernio mantido como está.** O adapter (`lib/publishing/zernio.ts`) foi escrito inicialmente sem documentação real da API (stub best-effort do M7) e **reescrito em 09/07/2026** contra a doc pública real (`docs.zernio.com`, início do M12) — ver detalhe técnico acima. Ainda **não testado contra uma conta Zernio real** (Victor ainda não criou a conta) — a auditoria de código está feita, falta a validação end-to-end do M12 (criar conta, conectar `@puzzlerecordss`, publicar de verdade). A `PublishingProvider` isolada garante que ajustes finais pós-teste real, ou uma troca de fornecedor, sejam um ajuste, não um redesenho.
