...
- **Zernio validado com publicação real.** O adapter (`lib/publishing/zernio.ts`) foi escrito inicialmente sem documentação real da API (stub best-effort do M7), reescrito contra a doc pública (`docs.zernio.com`) e depois **testado com 3 publicações reais bem-sucedidas no Instagram** (conta de teste `@althorya.ai`, 09/07/2026) — ver detalhe técnico acima. 3 divergências entre a doc pública e o comportamento real foram corrigidas nesse processo (campo `filename` obrigatório no presign, resposta assíncrona de `POST /posts`, e um bug de código nosso com `??` vs `||` numa env var vazia); o débito de idempotência descoberto na validação também já foi fechado (`onSubmitted`/`resolvePendingPublish`, validado numa 4ª publicação real). Ainda falta: testar vídeo e conectar a conta oficial `@puzzlerecordss` antes do go-live real. A `PublishingProvider` isolada garante que esses ajustes, ou uma troca de fornecedor, sejam incrementais, não um redesenho.
